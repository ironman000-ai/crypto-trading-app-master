...
export default function AutoTradePage() {
  ...
  const connectToAPI = async () => {
    ...
    try {
      ...
      // 模拟获取账户信息
      const mockAccountData: LiveAccountData = {
        totalBalance: 5000 + Math.random() * 10000,
        availableBalance: 3000 + Math.random() * 5000,
        positions: [
          {
            coin: 'BTC',
            amount: 0.1 + Math.random() * 0.5,
            entryPrice: 44000 + Math.random() * 2000,
            currentPrice: marketData.find(m => m.coin === 'BTC')?.price || 45000,
            profit: 0,
            profitPercent: 0
          },
          {
            ...
            entryPrice: 2700 + Math.random() * 200,
            currentPrice: marketData.find(m => m.coin === 'ETH')?.price || 2800,
            profit: 0,
            profitPercent: 0
          }
        ],
      ...
      // 计算持仓盈亏
      mockAccountData.positions = mockAccountData.positions.map(pos => {
        ...
        return { ...pos, profit, profitPercent };
      });

      setLiveAccountData(mockAccountData);
      setApiConnected(true);
      toast.success(`成功连接到${apiCredentials.exchange.toUpperCase()} API`);
    } catch (error) {
      toast.error('API连接失败，请检查密钥配置');
    } finally {
  ...
  const syncAccountData = async () => {
    ...
    try {
      // 模拟API调用获取最新账户数据
      const updatedPositions = liveAccountData.positions.map(pos => {
        const currentMarketData = marketData.find(m => m.coin === pos.coin);
        if (currentMarketData) {
          const currentPrice = currentMarketData.price;
          const profit = (currentPrice - pos.entryPrice) * pos.amount;
          const profitPercent = ((currentPrice - pos.entryPrice) / pos.entryPrice) * 100;
          return { ...pos, currentPrice, profit, profitPercent };
        }
        return pos;
      });

      setLiveAccountData(prev => ({
        ...prev,
        positions: updatedPositions,
        lastUpdated: new Date().toISOString()
      }));
    } catch (error) {
      console.warn('账户数据同步失败:', error);
    }
  };
  ...
  useEffect(() => {
    ...
    const syncInterval = setInterval(syncAccountData, 10000); // 每10秒同步一次
    return () => clearInterval(syncInterval);
  }, [apiConnected, marketData, liveAccountData.positions]);
